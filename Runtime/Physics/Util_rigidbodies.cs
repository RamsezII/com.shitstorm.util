...
partial class Util
{
    ...
    public static Quaternion SmoothDampRotation_degrees(in Quaternion current, in Quaternion target, ref Vector3 angularVelocity, in float damp, in float maxSpeed, in float deltaTime)
    {
    ...
    public static Quaternion SmoothDampRotation_radians(in Quaternion current, in Quaternion target, ref Vector3 angularVelocity, in float damp, in float maxSpeed, in float deltaTime)
    {
        Vector3 eulers = (target * Quaternion.Inverse(current)).eulerAngles.SignedEulers() * Mathf.Deg2Rad;
        Vector3.SmoothDamp(Vector3.zero, eulers, ref angularVelocity, damp, maxSpeed, deltaTime);
        return Quaternion.Euler(Mathf.Rad2Deg * deltaTime * angularVelocity) * current;
    }

    public static void SmoothDampRotation_radians(this Rigidbody rigidbody, in Quaternion target, in float damp, in float maxSpeed, in float control)
    {
        Vector3 angularVelocity1 = rigidbody.angularVelocity;
        Vector3 angularVelocity2 = angularVelocity1 * Mathf.Rad2Deg;
        SmoothDampRotation_degrees(rigidbody.rotation, target, ref angularVelocity2, damp, maxSpeed, Time.fixedDeltaTime);
        rigidbody.AddTorque(control * (angularVelocity2 * Mathf.Deg2Rad - angularVelocity1), ForceMode.VelocityChange);
    }

    public static void SmoothDampPosition(this Rigidbody rigidbody, in Vector3 target, in float damp, in float maxSpeed, in float control)
    {
